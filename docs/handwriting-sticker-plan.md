...
![AI 手写字成片效果：同一组夜景照片中的多种笔迹、排版、颜色与外发光](assets/editor-handwriting-gallery.jpg)

### 成片效果验收

- 手写字必须像摄影作品的一部分，而不是覆盖在照片上的系统字体；笔画需保留提按、连笔、飞白或自然不齐等真实书写特征。
- 同一条生成管线应能覆盖粗毛笔、细硬笔与轻松手账体等差异明显的笔迹，并支持多行错落排版。
- 抠图后不应残留纸张底色；纯色或渐变换色、描边、阴影与外发光均需保持细笔画和半透明笔触。
- 最终效果应可直接用于成片：复杂夜景中仍清晰可读，同时不过度遮挡主体和环境光。
