...
3. **国家、城市（schema 9）。** 每条位置记录在写入时用离线地名库反查出规范的国家（ISO 代码）和城市（地名库自己的城市：id、英文名、简体中文名），所以 GPS、AI 猜测、手动标记落在同一组选项上。值存规范名，界面按语言显示。AI 记录精度只到省或国家时不给城市（坐标是区域中心点，说明不了城市）。升级时对已有位置记录回填一次；升级前自动把数据库复制一份（`catalog.sqlite3.schema8.bak`）。已知取舍：反查按「重要性减距离」打分，大城市旁边的小城市可能被并进大城市（泽西城算进纽约）。web 版没有地名库，这两个筛选不出现。
