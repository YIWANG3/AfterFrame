...
1. **单片比例**：与 `CropPanel` 相同的迷你矩形按钮，只保留竖向 / 方形常用项：3:4（默认，竖构图）、4:5、1:1、9:16、2:3；下方一行「自定义」W:H 数字输入（1..100），失焦或回车提交，最近一次的自定义值随状态保留。改比例或片数时区域保持中心、取能放下的最大尺寸（与裁剪工具改比例一致）。
