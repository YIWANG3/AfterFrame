...
1. **长宽比**：与裁剪工具的「长宽比」区块完全一致（同一标题、同一 `ASPECT_PRESETS` 列表、同一 `AspectButton`）：Free、Original、1:1、3:2、2:3、4:3、3:4（默认）、5:4、4:5、16:9、9:16、2.35:1。Free 时区域不锁比例，八个手柄自由拉，每片 = 区域宽 / N；Original 时单片比例等于原图比例。改比例或片数时区域保持中心、取能放下的最大尺寸（Free 时保持现有形状）。
