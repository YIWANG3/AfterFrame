...
1. **长宽比（每片）**：标题、按钮样式与裁剪工具的「长宽比」一致，但语义是单片比例，预设只保留竖向 / 方形：1:1、3:4（默认）、2:3、9:16、4:5；第六个格子是「自定义」，虚线图标加两个原地输入框「宽 : 高」（样式同拼图面板的比例输入），输入即生效、聚焦即选中，不需要点开。改比例或片数时区域保持中心、取能放下的最大尺寸。
