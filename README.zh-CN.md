...
- **备份与迁移**：把设置搬到另一台 Mac。AI 服务商配置、API 密钥和应用偏好导出为一个口令加密的文件（设置 → 通用）；密钥由本机钥匙串加密，直接拷贝设置文件到新电脑是解不开的
...
### 编辑
- **裁剪**：预设比例、旋转、翻转
- **切图**：把宽图无缝切成 N 张竖图，直接发社媒轮播。单片比例预设 3:4、4:5、1:1、9:16、2:3 或自定义，片数自动或手动，覆盖区域可拖动缩放并跟随校正角度，按原始分辨率导出到 `<文件名>_split` 文件夹，每一张都进入原图的版本堆栈
