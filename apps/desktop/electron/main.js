...
let cachedSwiftRuntime = null;
function findSwiftRuntime() {
  if (cachedSwiftRuntime) return cachedSwiftRuntime;
  ...
  if (fs.existsSync(xcodeSwift)) {
    cachedSwiftRuntime = { binary: xcodeSwift, developerDir: xcodeDeveloper };
    return cachedSwiftRuntime;
  }
  cachedSwiftRuntime = { binary: "/usr/bin/swift", developerDir: null };
  return cachedSwiftRuntime;
}

// Per-image cache so the same source isn't re-inferred. Key includes size+mtime
// so editing the file invalidates the cached depth automatically.
