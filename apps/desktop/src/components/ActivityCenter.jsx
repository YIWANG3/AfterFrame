...
export function jobLine(job) {
  ...
  if (total > 0) return `${job.phaseLabel || "Working"} · ${processed}/${total}`;
  return job.phaseLabel || (job.status === "queued" ? "Queued" : "Working…");
}
  ...
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5 text-[11px] text-muted2">
      ...
      <span className="min-w-0 flex-1 truncate">
        {meta.label} {job.status === "succeeded" ? "finished" : job.status}
      </span>
    </div>
  );
}
    ...
    <>
      ...
      {open && rect && createPortal(
        ...
        >
          ...
          {count === 0 ? (
            <div className="px-3 py-4 text-center text-[11px] text-muted2">No background jobs running</div>
          ) : (
                ...
                return (
                  <div key={job.jobId} className="px-2.5 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="min-w-0 flex-1 truncate text-[11px] font-medium text-text">{meta.label}</span>
                    ...
                    <div className="mt-1 truncate pl-[22px] text-[10px] text-muted2">
                      {cancelling ? "Cancelling…" : jobLine(job)}
                    </div>
